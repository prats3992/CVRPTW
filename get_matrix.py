...
def create_time_matrix(data):
  ...
  for i in range(q):
    origin_addresses = addresses[i * max_rows: (i + 1) * max_rows]
    response = send_request(origin_addresses, dest_addresses, API_key)
    time_matrix += build_matrix(response, type='duration')
...
def build_matrix(response,type='distance'):
  distance_matrix = []
  for row in response['rows']:
    row_list = [row['elements'][j][type]['value'] for j in range(len(row['elements']))]
